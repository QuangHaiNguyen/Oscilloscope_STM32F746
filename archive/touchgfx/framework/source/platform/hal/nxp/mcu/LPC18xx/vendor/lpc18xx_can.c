/**********************************************************************
* $Id: lpc18xx_can.c 48117 2011-10-20 12:13:01Z anderslu $              lpc18xx_can.c           2011-06-02
*//**
* @file         lpc18xx_can.c
...
#define CAN_IF_Read(reg, IFsel) (LPC_C_CAN->##IFsel##_##reg)
#define CAN_IF_Write(reg, IFsel, val) (LPC_C_CAN->##IFsel##_##reg=val)

/* Macro for writing IF to specific RAM message object */
#define CAN_IF_readBuf(IFsel,msg) \
  LPC_C_CAN->##IFsel##_##CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB; \
  LPC_C_CAN->##IFsel##_##CMDREQ=msg; \
  while (LPC_C_CAN->##IFsel##_##CMDREQ & IFCREQ_BUSY );

/* Macro for reading specific RAM message object to IF */
#define CAN_IF_writeBuf(IFsel,msg) \
  LPC_C_CAN->##IFsel##_##CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB; \
  LPC_C_CAN->##IFsel##_##CMDREQ=msg; \
  while (LPC_C_CAN->##IFsel##_##CMDREQ & IFCREQ_BUSY );
...
void CAN_IRQHandler(void)
{
        ...
        while ( (can_int = LPC_C_CAN->INT) != 0 )       /* While interrupt is pending */
        {
                canstat = LPC_C_CAN->STAT;                              /* Read CAN status register */
                ...
                if ( can_int & CAN_STATUS_INTERRUPT )
                {
                        ...
                        LPC_C_CAN->STAT &= STAT_RXOK|STAT_TXOK;
                }
                else
                {
...
void CAN_Init( uint32_t BitClk, CCAN_CLKDIV_Type ClkDiv , MSG_CB Tx_cb, MSG_CB Rx_cb)
{
        ...
        RX_cb = Rx_cb;
        TX_cb = Tx_cb;
        if ( !(LPC_C_CAN->CNTL & CTRL_INIT) )
        {
                /* If it's in normal operation already, stop it, reconfigure
                 everything first, then restart.  */
                LPC_C_CAN->CNTL |= CTRL_INIT;   /* Default state */
        }

        LPC_C_CAN->CLKDIV = ClkDiv;                     /* Divider for CAN VPB3 clock */
        LPC_C_CAN->CNTL |= CTRL_CCE;            /* Start configuring bit timing */
        LPC_C_CAN->BT = BitClk;
        LPC_C_CAN->BRPE = 0x0000;
        LPC_C_CAN->CNTL &= ~CTRL_CCE;           /* Stop configuring bit timing */

        LPC_C_CAN->CNTL &= ~CTRL_INIT;          /* Initialization finished, normal operation now. */
        while ( LPC_C_CAN->CNTL & CTRL_INIT );

        /* By default, auto TX is enabled, enable all related interrupts */
        LPC_C_CAN->CNTL |= (CTRL_IE|CTRL_SIE|CTRL_EIE);
        return;
}
