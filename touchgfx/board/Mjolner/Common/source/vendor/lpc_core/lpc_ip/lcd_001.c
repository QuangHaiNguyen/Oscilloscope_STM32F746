...
/*void IP_LCD_Init(IP_LCD_001_T *pLCD, LCD_Config_T *LCD_ConfigStruct)
{
        ...
        default:
                regValue |=      
                //1<<26 | 
                (((LCD_ConfigStruct->PPL - 1) & 0x3FF) << 16);
        }
...
void IP_LCD_Init(IP_LCD_001_T *pLCD, LCD_Config_T *LCD_ConfigStruct)
{
        ...
        if(LCD_ConfigStruct->line_end_delay)
                pLCD->LE  = (LCD_ConfigStruct->line_end_delay-1) | 1<<16;
        else
                pLCD->LE = 0;
        
        // panel clock divisor  
        pcd = 1;// LCD_ConfigStruct->pcd;   // TODO: should be calculated from LCDDCLK
        pcd &= 0x3FF;
        regValue |=  ((pcd >> 5) << 27) | (1 << 26) | ((pcd) & 0x1F);
        pLCD->POL = regValue;
