...
int RingBuffer_Insert(RINGBUFF_T *RingBuff, const void *data)
{
        ...
        /* We cannot insert when queue is not valid or full */
        if (!RingBuffer_IsValid(RingBuff) || RingBuffer_IsFull(RingBuff))
                return 0;
...
int RingBuffer_Pop(RINGBUFF_T *RingBuff, void *data)
{
        ...
        /* We cannot pop when queue is not valid or empty */
        if (!RingBuffer_IsValid(RingBuff) || RingBuffer_IsEmpty(RingBuff))
                return 0;
...
int RingBuffer_PopMult(RINGBUFF_T *RingBuff, void *data, int num)
{
        ...
        /* We cannot insert when queue is not valid or empty */
        if (!RingBuffer_IsValid(RingBuff) || RingBuffer_IsEmpty(RingBuff))
                return 0;
